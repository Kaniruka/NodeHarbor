...
2. 打开“订阅”，添加订阅源。可以粘贴订阅 URL、上传 YAML 文件，或直接粘贴 YAML 内容。URL 订阅可选填写 User-Agent；URL 返回的普通 Clash/Mihomo YAML、Base64 编码 YAML，以及常见的 Base64 `ss://`、`vmess://`、`vless://`、`trojan://`、`hysteria://`、`hysteria2://`、`anytls://` 节点列表都会自动处理。
...
Expand-Archive .\dist\NodeHarbor-windows-0.3.3.zip -DestinationPath .\dist\nodeharbor-package
