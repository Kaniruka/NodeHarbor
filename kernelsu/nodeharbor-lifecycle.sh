...
nodeharbor_start() {
  ...
  if [ -r "$PID_FILE" ]; then
    pid=$(cat "$PID_FILE" 2>/dev/null)
    if nodeharbor_pid_is_owned "$pid"; then return 0; fi
    rm -f "$PID_FILE"
  fi
  if [ -n "${NODEHARBOR_LISTEN:-}" ]; then
    nohup "$NODEHARBOR_BIN" --listen "$NODEHARBOR_LISTEN" --data "$DATA_DIR" --open-browser=false >"$LOG_DIR/nodeharbor.log" 2>&1 &
  else
    nohup "$NODEHARBOR_BIN" --data "$DATA_DIR" --open-browser=false >"$LOG_DIR/nodeharbor.log" 2>&1 &
  fi
  echo $! >"$PID_FILE"
  sleep 1
